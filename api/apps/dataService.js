...
var App = {
	...
	cityCommittees: async function(){
		var model = {url: 'https://gist.githubusercontent.com/mayurah/5f4a6b18b1aa8c26910f/raw/8dd2b9486874d283141cad8cccc5916e48c75dcd/countriesToCities.json', json: true};
	...
	getLegislators: async function(lat, lng){
		...
		var stateModel= {url: 'http://openstates.org/api/v1/legislators/geo/?lat='+lat+'&long='+lng+'&active=true&apikey='+openCongressApiKey, json: true};
		var federalModel = {url: 'http://congress.api.sunlightfoundation.com/legislators/locate?latitude='+lat+'&longitude='+lng+'&per_page=all&apikey='+propublicaApiKey, json: true};
	...
	nationalCommittees: async function(){
		...
		for(x in countryData){
			...
			var urlTitle = title.replace(/ /g,"-").toLowerCase();
			var model = {parent: null, title: title, urlTitle: urlTitle};
	...
	stateCommittees: async function(){
		var model = {url: 'https://openstates.org/api/v1//committees/?apikey=' + openCongressApiKey, json: true,};
		var body = await rp(model);
	    if (body) {
			...
			for (x in committeeDataArray){
				...
				var model = {url: 'http://openstates.org/api/v1/committees/' + committeeData.id + '?apikey=' + openCongressApiKey, json: true,};
				var body = await request(model);
				...
				else{var updatedCommittee = await Committee.update({officialId: officialId}, model);}
			};
		}
	},
	...
	stateLegislators: async function(){
	    ...
	    if (body) {
			...
			for (x in stateData) {
				...
				var trim_first_name = first_name.trim().replace(/\s+/g, '.').replace(/"/g,'').replace(/,/g, '').replace(/\\/g, '')
				var trim_last_name = last_name.trim().replace(/\s+/g, '.').replace(/"/g,'').replace(/,/g, '').replace(/\\/g, '');
				var username = trim_first_name + '.' + trim_last_name;
				var trim_username = username.replace(/[()]/g, '').replace(/\.\./g, '.')
				var email =  stateData[x].email;
				if( typeof email === 'undefined' || email === null || typeof email === 'string' ){email = trim_username + '@gmail.com';}
	...
	stateVotes: async function(state, bill, votes){
		...
		for (x in votes){
			...
			var yesVotesArray = _.map(votes[x].yes_votes, function(element) { return _.extend({}, element, {vote: 1, voteString: 'Yea'});});
			var noVotesArray = _.map(votes[x].no_votes, function(element) { return _.extend({}, element, {vote: -1, voteString: 'Nay'});});
			var otherVotesArray = _.map(votes[x].other_votes, function(element) { return _.extend({}, element, {vote: 0, voteString: 'No Vote'});});
			var votesArray = [];
	...
	federalBills: async function(pageStart, pageEnd){
		for (var page = pageStart; page <= pageEnd; page++){
			var model = {url: 'https://congress.api.sunlightfoundation.com/bills?&per_page=1&page=' + page + '&fields=actions,bill_id,bill_type,committee_ids,congress,keywords,number,official_title,related_bill_ids,short_title,sponsor,summary,summary_short,urls,upcoming&apikey=' + openCongressApiKey, json: true,};
			var body = await rp(model);
			if (body) {
	...
	federalCommittees: async function(){
		var model = {url: 'https://congress.api.sunlightfoundation.com/committees?fields=chamber,committee_id,members,name,parent_committee_id&per_page=all&apikey=' + openCongressApiKey, json: true};
		var body = await rp(model);
		if (body.results) {
    		...
    		for (x in committeeDataArray){
				...
				var unitedStatesModel = {title: 'United States', urlTitle: 'united-states', parent: null};
				var newCommittee;
				...
				if (newCommittee){
					var newLegCommitteeModel = {title: newCommittee, urlTitle: newCommittee.toLowerCase().replace(/ /g,'-'), parent: committeeModel.id};
					...
					if(members.length != 0){
						for (x in members){
							...
							if (title == null){title = 'Committee Member'}
							var committeeMemberModel = {committee: committeeModel.id, title: title, user: userModel[0].id};
							...
							var committeeMemberModel = {committee: committee[0].id, title: userModel[0].title, user: userModel[0].id};
							var committeeMember = await CommitteeMember.findOrCreate({committee: committee[0].id, user: userModel[0].id}, committeeMemberModel)
							console.log('COMMITTEE MEMBER CREATED US');
							CommitteeMember.publishCreate(committeeMember);
	...
	federalLegislators: async function(){
		var model = {url: 'http://congress.api.sunlightfoundation.com/legislators?per_page=all&apikey=' + openCongressApiKey, json: true};
		var body = await rp(model);
	    if (body.results) {
	...
	federalVotes: async function(bill){
		var model = {url: 'https://congress.api.sunlightfoundation.com/votes?bill_id=' + bill.officialId + '&fields=breakdown,question,required,result,roll_id,url,vote_type&apikey=' + openCongressApiKey, json: true,};
		var body = await rp(model);
	...
	federalVoteVotes: async function(vote){
		var model = {url: 'https://congress.api.sunlightfoundation.com/votes?roll_id=' + vote.officialId + '&fields=voters&apikey=' + openCongressApiKey, json: true};
		var body = await rp(model);
		if (body && body.results) {
			if (body.results.length>0){
				...
				for (x in voteVoteData){
					...
					if(user[0]){user = userModel[0].id}
					var model = {voteInteger: voteInteger, voteString: voteVoteModel.vote, vote: vote.id, bill: vote.bill, user: user};
					var voteVoteModel = await VoteVote.findOrCreate({bill: model.bill, vote: model.vote, user: model.user}, model)
					VoteVote.publishCreate(voteVoteModel);
					//userModel.-->twitter
					//vote.bill
