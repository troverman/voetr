...
var App = {
	...
	loadTemplates: function(){
		...
		return utilsService.promisify(fs.readdir, "./api/views/email/")
		.then(function(fileNames){
			templateFileNames = fileNames.map(function(fileName){return fileName.split(".").shift()});
			return Promise.all(fileNames.map(function(fileName){return utilsService.promisify(fs.readFile, "./api/views/email/" + fileName);}))
		})
		.then(function(files){
			templateFileNames.forEach(function(name, i){that.templates[name] = Handlebars.compile(files[i].toString())})
			return true;
		})
	},
	templates: {},
